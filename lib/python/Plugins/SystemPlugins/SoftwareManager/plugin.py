...
from Tools.HardwareInfo import HardwareInfo #IQON
	...
	def JobStart(self):
		...
		try:
			...
			if os_path.exists(self.BackupDirectory + config.plugins.configurationbackup.folderprefix.value + "-swapfile_backup"):
				os_system('swapoff ' + self.BackupDirectory + config.plugins.configurationbackup.folderprefix.value + "-swapfile_backup")
...
#		self.BackupDate = getImageVersionString() + '.' + getBuildVersionString() + '-' + strftime('%Y%m%d_%H%M%S', localtime())
	...
	def doBackup2(self):
		...
		if HardwareInfo().get_device_name() in ("tmnano", "optimussos1", "optimussos1plus", "tmnanosuper", "force2", "tmnano2t" ):
			self.command = 'cat /dev/mtd1 > ' + self.WORKDIR + '/vmlinux.gz'
		elif HardwareInfo().get_device_name() in ("force1", "tm2tsuper", "optimussos2plus", "force1plus", "tmnano2tsuper"):
...
class ImageRestore(Screen):
	...
	def doRestore(self,answer):
		if answer:
			...
			if HardwareInfo().get_device_name() in ("force1", "tm2tsuper", "force1plus", "optimussos2plus", "tmnano2tsuper"):
				kernelMTD = "mtd2"
				kernelFILE = "oe_kernel.bin"
				rootMTD = "mtd0"
				rootFILE = "oe_rootfs.bin"
			elif HardwareInfo().get_device_name().startswith("tmnano"):
				kernelMTD = "mtd1"
				kernelFILE = "oe_kernel.bin"
				rootMTD = "mtd0"
				rootFILE = "oe_rootfs.bin"
