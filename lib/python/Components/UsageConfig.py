...
# ]
# [ 20130808 
	config.usage.activate_ci_setup = ConfigYesNo(default = False)
# ]
...
#	config.usage.show_cryptoinfo = ConfigYesNo(default = True)
	config.usage.show_cryptoinfo = ConfigSelection(default = "emu", choices = [ "full", "emu", "off" ])
# ]
	...
	EpgSettingsChanged(config.epg.eit)	# iq
	...
	config.subtitles.pango_subtitles_yellow = ConfigSelection(default = "0", choices = [
		("0", _("alternative")),
		("1", _("white")),
		("2", _("yellow")) ])
...
#	config.autolanguage.equal_languages = ConfigSelection(default = "15", choices = [
	...
	# ci enable for ci model.
	config.active_ci_path = ConfigYesNo(default = False)
