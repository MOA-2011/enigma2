...
#iq ]
				"openProviderList": (self.openProviderList, _("open ProviderList")),				
			})
...
class InfoBarEPG:
		...
		self["EPGActions"] = HelpableActionMap(self, "InfobarEPGActions",
			{
				...
				"showInfobarOrEpgWhenInfobarAlreadyVisible": self.showEventInfoWhenNotVisible,
				"showYouTube": (self.showYouTube, _("Show YouTube...")),
			})
	...
	def showYouTube(self):
		try:
			...
			self.session.open(MyTubePlayerMainScreen)
			no_plugin = False
		except Exception, e:
			from Screens.MessageBox import MessageBox
			self.session.open(MessageBox, _("The MediaPlayer plugin in not installed!\nPlease install it."), type = MessageBox.TYPE_INFO, timeout = 10)		
