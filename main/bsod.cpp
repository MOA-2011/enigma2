...
#define CRASH_EMAILADDR "forum at www.technomate-community-forum.com"
...
void bsodFatal(const char *component)
{
	...
	// technomate crash log mode enable.
#if 0 //TMTWIN
	...
	if (f)
	{
...
#else
		// technomate model checkfile is "/proc/stb/info/hwmodel" : by han
		if (access("/proc/stb/info/hwmodel", F_OK) != -1) {
			xml.stringFromFile("stbmodel", "/proc/stb/info/hwmodel");
		}
#endif
		...
		if (detailedCrash)
		{
			...
			if(access("/proc/stb/info/boxtype", F_OK) != -1) {
				...
				xml.cDataFromCmd("tmsoftware", "opkg list-installed 'tm*'");
			}
